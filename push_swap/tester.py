...
def calculate_combination(nb):
	...
	for perm in permutations(elmt):
		...
		res_cmd = "./push_swap " + cmd + " | ./checker_OS " + cmd
	...
	if limit == 3:
		...
		all_too_much = None
	all_wrong_values = [("./push_swap " + item[1] + " | ./checker_OS " + item[1]) for item in all_res if item[0] == "KO\n"]
	if all_too_much:
		f = open('trace.txt', 'a+')
		if limit == 3:
...
def calculate(nb):
	...
	for x in range(0, 10):
		...
		res_cmd = "./push_swap " + nbs + " | ./checker_OS " + nbs	
		res_output = os.popen(res_cmd).read()
		if res_output == "KO\n":
			print(color.BOLD + f"TEST {x + 1} 👉 \t❌ " + color.RED + "KO" + color.END)
			wrong_values.append(("./push_swap " + nbs + " | ./checker_OS " + nbs))
		else:
...
if __name__ == "__main__":
	...
	if not os.path.exists("push_swap"):
		os.system("make")
	if not os.path.exists("checker_OS"):
		print(color.RED + "\nError: the tester needs checker_OS at the root of the repository!\n" + color.END)
		exit(1)
